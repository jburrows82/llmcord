...
async def generate_response_stream(
    ...
    if is_gemini:
        logging.debug(
            "Preparing history for Gemini API format from input history_for_llm."
        )
        for msg_index, original_msg_data in enumerate(history_for_llm):
            ...
            if "parts" in original_msg_data and isinstance(
                ...
                if isinstance(openai_content, str):
                    ...
                    ]  # Convert OpenAI string content to part structure
                elif isinstance(openai_content, list):
                    source_parts_or_content = (
                        openai_content  # Already a list of OpenAI parts
                    )
                else:
                    logging.warning(
                        f"Message {msg_index} (role '{original_msg_data['role']}') has 'content' of unexpected type {type(openai_content)}. Skipping."
                    )
                    continue
            else:
                logging.warning(
                    f"Message {msg_index} (role '{original_msg_data['role']}') has neither 'parts' nor 'content'. Skipping."
                )
                continue
            ...
            for part_idx, part_item in enumerate(source_parts_or_content):
                if isinstance(part_item, google_types.Part):
                    ...
                    part_item, str
                ):  # Should ideally be caught by OpenAI content wrapping
                    gemini_parts_for_this_msg.append(
            ...
            if gemini_parts_for_this_msg:
                history_to_use_for_api_call.append(
                    {
                ...
                logging.warning(
                    f"Message {msg_index} (role '{original_msg_data['role']}') resulted in no valid Gemini parts after conversion. Skipping."
                )
    ...
    for key_index, current_api_key in enumerate(keys_to_loop):
        ...
        try:  # Inner try for the specific API call attempt
            ...
            if is_gemini:
                ...
                for msg_data in history_to_use_for_api_call:
                    ...
                    if not parts_list:  # Should not happen if correction logic is sound and msg was not skipped
                        logging.warning(
                            f"Message with role '{role}' has no parts after correction. Skipping for Gemini contents."
                        )
                        continue
                    try:
                ...
                if not gemini_contents:  # If all messages were skipped or had no parts
                    ...
                    yield (
                        ...
                        None,
                        None,
                        "Internal error: No valid content to send to Gemini after history processing.",
                    )
                    return
                ...
                if gemini_thinking_budget_val is not None:
                    ...
                    if budget_to_apply is not None:
                        if (
                            0 <= budget_to_apply <= 24576
                        ):  # Max value for thinking_budget (as per Gemini docs)
                            if (
                ...
                payload_to_print["generationConfig"] = {
                    k: v for k, v in payload_to_print["generationConfig"].items() if v
                }
                ...
                try:  # Inner try for stream processing errors
                    if is_gemini:
                        ...
                        if hasattr(chunk, "text") and chunk.text:
                            ...
                            logging.debug(
                                f"Received Gemini chunk text (len: {len(new_content_chunk)})"
                            )
                        ...
                        if hasattr(chunk, "candidates") and chunk.candidates:
                            ...
                            if (
                                ...
                                logging.info(
                                    f"Gemini finish reason received: {chunk_finish_reason} ({candidate.finish_reason})"
                                )
                                ...
                                if chunk_finish_reason:
                                    ...
                                    if finish_reason_lower == "safety":
                                        logging.warning(
                                            f"Gemini Response Blocked (finish_reason=SAFETY) with key {key_display}. Check prompt/safety settings."
                                        )
                                        llm_errors.append(
                                        ...
                                        logging.warning(
                                            f"Gemini Response stopped due to Recitation (finish_reason=RECITATION) with key {key_display}."
                                        )
                                        llm_errors.append(
                                        ...
                                        logging.warning(
                                            f"Gemini Response Blocked (finish_reason=OTHER) with key {key_display}."
                                        )
                                        llm_errors.append(
                            ...
                            if (
                                ...
                                for rating in candidate.safety_ratings:
                                    if rating.probability in (
                                        ...
                                        logging.warning(
                                            f"Gemini content potentially blocked by safety rating: Category {rating.category}, Probability {rating.probability}. Key: {key_display}"
                                        )
                    ...
                    if stream_finish_reason:
                        logging.info(
                            f"Stream finished with reason: {stream_finish_reason}. Exiting inner loop."
                        )
                        break  # Exit inner stream loop

                except google_api_exceptions.GoogleAPIError as stream_err:
                    logging.warning(
                        f"Google API error during streaming with key {key_display}: {type(stream_err).__name__} - {stream_err}. Trying next key."
                    )
                    llm_errors.append(
                    ...
                    logging.warning(
                        f"Connection error during streaming with key {key_display}: {stream_err}. Trying next key."
                    )
                    llm_errors.append(
                    ...
                    logging.warning(
                        f"API error during streaming with key {key_display}: {stream_err}. Trying next key."
                    )
                    llm_errors.append(
                    ...
                    logging.exception(
                        f"Unexpected error during streaming with key {key_display}"
                    )
                    llm_errors.append(
            ...
            if (
                ...
                is_successful_empty_finish = stream_finish_reason and (
                    stream_finish_reason.lower() == "stop"
                    or (
                        is_gemini
                ...
                if is_successful_empty_finish:
                    logging.warning(
                        f"LLM stream finished successfully for key {key_display} but NO content was received. Finish reason: {stream_finish_reason}. Aborting retries."
                    )
                    llm_errors.append(
                        f"Key {key_display}: No content received (Successful Finish)"
                    )
                    ...
                    yield (
                        ...
                        f"No content received (Successful Finish: {stream_finish_reason})",
                    )
                    return  # Stop generation
                elif stream_finish_reason:
                    logging.warning(
                        f"LLM stream finished with reason '{stream_finish_reason}' for key {key_display} but NO content was received. Aborting retries."
                    )
                    llm_errors.append(
                        f"Key {key_display}: No content received (Finish Reason: {stream_finish_reason})"
                    )
                    ...
                    logging.warning(
                        f"LLM stream ended prematurely for key {key_display} and NO content was received. Last error type: {last_error_type}. Aborting retries."
                    )
                    if not llm_errors or not llm_errors[-1].startswith(
                        f"Key {key_display}"
                    ):
                        llm_errors.append(
                            f"Key {key_display}: Stream ended prematurely with no content"
                        )
                    ...
                    yield (
                        ...
                        stream_finish_reason,
                        stream_grounding_metadata,
                        "Stream ended prematurely with no content",
                    )
                    return  # Stop generation

            elif is_blocked_by_safety:
                logging.warning(
                    f"LLM response blocked due to safety/other with key {key_display}. Aborting retries."
                )
                yield (
                    None,
                    "safety",
                ...
                logging.warning(
                    f"LLM response stopped due to recitation with key {key_display}. Aborting retries."
                )
                yield (
                    None,
                    "recitation",
                    stream_grounding_metadata,
                    "Response stopped due to recitation.",
                )
                return  # Stop generation
                ...
                if is_successful_finish:
                    ...
                    logging.warning(
                        f"LLM stream finished with non-stop reason '{stream_finish_reason}' for key {key_display}. Trying next key."
                    )
                    llm_errors.append(
                        f"Key {key_display}: Finished Reason '{stream_finish_reason}'"
                    )
                ...
                logging.warning(
                    f"LLM stream broke during processing for key {key_display}. Last error type: {last_error_type}. Trying next key."
                )
                continue  # Try next key
                ...
                if (
                    ...
                    logging.warning(
                        f"Non-Gemini LLM stream for key {key_display} ended without finish reason but with content. Signaling for Gemini retry."
                    )
                    yield (
                        None,
                        None,
                ...
                logging.warning(
                    f"LLM stream ended without a finish reason for key {key_display}, but content was received. Treating as successful completion. Content received: {content_received}, Finish reason: {stream_finish_reason}, Blocked: {is_blocked_by_safety}, Stopped: {is_stopped_by_recitation}."
                )
                # Treat as successful completion since content was received
                logging.info(
                    f"LLM request considered successful (stream ended without finish reason) with key {key_display}"
                )
        ...
        # --- Handle Initial API Call Errors ---
            ...
            logging.error(
                f"Initial Request Authentication failed for provider '{provider}' with key {key_display}. Error: {e}. Aborting retries."
            )
            ...
            logging.error(
                f"Initial Request Bad request error for provider '{provider}' with key {key_display}. Error: {e}. Aborting retries."
            )
            if isinstance(e, google_api_exceptions.InvalidArgument):
                error_details = getattr(e, "details", None)
                if error_details:
                    logging.error(
                        f"Google API InvalidArgument Details: {error_details}"
                    )
        ...
        ) as e:  # Catch other Google API errors
            logging.exception(
                f"Initial Request Google API Call Error for key {key_display}"
            )
            error_details = getattr(e, "details", None)
            if error_details:
                logging.error(f"Google API Error Details: {error_details}")
            llm_errors.append(
    ...
    logging.error(
        f"All LLM API keys failed for provider '{provider}'. Errors: {llm_errors}"
    )
