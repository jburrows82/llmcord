...
from .constants import (
    GROUNDING_MODEL_PROVIDER, GROUNDING_MODEL_NAME,
    SEARXNG_BASE_URL_CONFIG_KEY, SEARXNG_NUM_RESULTS,
    SEARXNG_URL_CONTENT_MAX_LENGTH_CONFIG_KEY, AllKeysFailedError
)
from .content_fetchers import (
    fetch_searxng_results, fetch_youtube_data, fetch_reddit_data,
...
async def get_web_search_queries_from_gemini(
    ...
    gemini_provider_config = config.get("providers", {}).get(GROUNDING_MODEL_PROVIDER, {})
    ...
    try:
        ...
        grounding_extra_params = {"temperature": 1, "thinking_budget": 0} # Could be configurable
    ...
    except AllKeysFailedError as e: # Make sure AllKeysFailedError is accessible or handled
...
async def fetch_and_format_searxng_results(
    ...
    for query in queries:
        searxng_tasks.append(
            fetch_searxng_results(query, httpx_client, searxng_base_url, SEARXNG_NUM_RESULTS)
        )
        ...
        for url_from_searxng_for_query in urls_for_this_query_or_exc:
            ...
            if content_result and content_result.content and not content_result.error:
                ...
                logging.warning(f"Skipping SearxNG URL {content_result.url} for query '{query_str}' due to fetch error: {content_result.error}")
                 ...
                 logging.warning(f"Content for SearxNG URL {url_from_searxng_for_query} (query: '{query_str}') not found in pre-fetched map.")
