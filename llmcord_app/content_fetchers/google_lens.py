...
async def process_google_lens_image(image_url: str, index: int, cfg: Dict[str, Any]) -> UrlFetchResult:
    ...
    if serpapi_keys_ok:
        logging.info(f"Attempting Google Lens request for image {index+1} using SerpAPI.")
        serpapi_result = await fetch_google_lens_serpapi(image_url, index, serpapi_keys)

        if not serpapi_result.error:
            logging.info(f"SerpAPI Google Lens request successful for image {index+1}.")
            return serpapi_result
        else:
            serpapi_error_message = serpapi_result.error
            logging.warning(f"SerpAPI Google Lens request failed for image {index+1}: {serpapi_error_message}.")
    else:
         logging.info("SerpAPI keys not configured. Skipping Google Lens request.")

    # If SerpAPI was not configured or failed
    final_error_msg = f"Google Lens processing failed. SerpAPI Error: {serpapi_error_message}"
    return UrlFetchResult(url=image_url, content=None, error=final_error_msg, type="google_lens_serpapi", original_index=index)
