...
async def fetch_general_url_content(url: str, index: int, max_text_length: Optional[int] = None) -> UrlFetchResult:
    ...
    try:
                ...
                async for chunk in response.aiter_bytes():
                    ...
                    try:
                        html_content += chunk.decode(response.encoding or 'utf-8', errors='replace')
                         ...
                         logging.warning(f"Unicode decode error reading chunk from {url}: {decode_err}. Skipping chunk.")
                         continue # Skip this chunk

                    if len(html_content) > html_limit:
                        logging.warning(f"HTML content truncated for URL {url} due to size limit ({html_limit} bytes).")
                 ...
                 return UrlFetchResult(url=url, content=None, error=f"Content reading error: {type(e).__name__}", type="general", original_index=index)

        logging.debug(f"Successfully read HTML content (length: {len(html_content)}) for URL: {url}")
        ...
        try:
            ...
            content = text
            logging.debug(f"Extracted text content (length: {len(content)}) for URL: {url}")
            ...
            return UrlFetchResult(url=url, content=content, type="general", original_index=index)
             ...
             return UrlFetchResult(url=url, content=None, error=f"HTML parsing error: {type(parse_err).__name__}", type="general", original_index=index)
        ...
        return UrlFetchResult(url=url, content=None, error=error_msg, type="general", original_index=index)
    except Exception as e:
