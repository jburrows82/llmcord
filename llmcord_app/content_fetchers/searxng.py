...
async def fetch_searxng_results(
    ...
    try:
        logging.info(f"Querying SearxNG ('{query}') at {search_url} with params {params}")
        ...
        logging.info(f"SearxNG returned {len(urls)} URLs for query '{query}'. Requested up to {num_results}.")

    except httpx.HTTPStatusError as e:
        logging.error(f"SearxNG HTTP error for query '{query}': {e.response.status_code} - {e.response.text}")
    except httpx.RequestError as e:
        logging.error(f"SearxNG request error for query '{query}': {type(e).__name__} - {e}")
    except ValueError as e: # Includes JSONDecodeError
        logging.error(f"SearxNG JSON decode error for query '{query}': {e} (Response: {response.text[:200]}...)")
    except Exception as e:
        logging.exception(f"Unexpected error fetching SearxNG results for query '{query}'")
