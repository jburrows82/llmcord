...
def initialize_ytt_api(proxy_config_data: Optional[Dict] = None):
    ...
    if proxy_config_data and isinstance(proxy_config_data, dict):
        ...
        if proxy_type == "webshare" and WebshareProxyConfig and proxy_config_data.get("username") and proxy_config_data.get("password"):
            try:
        ...
        elif proxy_type == "generic" and GenericProxyConfig and (proxy_config_data.get("http_url") or proxy_config_data.get("https_url")):
            try:
            ...
            logging.warning(f"Invalid or incomplete proxy type '{proxy_type}' in config.yaml. Not using proxy.")
        # else: No proxy type specified or config is empty/invalid

    try:
        ytt_api = YouTubeTranscriptApi(proxy_config=ytt_proxy_config)
        logging.info(f"YouTubeTranscriptApi initialized {'with proxy' if ytt_proxy_config else 'without proxy'}.")
    except Exception as e:
        logging.error(f"Failed to initialize YouTubeTranscriptApi: {e}. Transcript fetching may fail.")
...
async def get_transcript(video_id: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        if 'requestblocked' in e_str or 'ipblocked' in e_str or 'too many requests' in e_str:
             logging.error(f"YouTube blocked transcript request for {video_id}: {type(e).__name__}: {e}")
             return None, "YouTube blocked the transcript request (IP ban or rate limit)."
        logging.error(f"Error fetching transcript for {video_id}: {type(e).__name__}: {e}", exc_info=True)
...
async def get_youtube_video_details(video_id: str, api_key: Optional[str]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    try:
        ...
        def sync_get_details():
            ...
            try:
                ...
                try:
                    ...
                    _errors.append(f"Unexpected error getting video details: {type(e_vid_other).__name__}")
                ...
                try:
                    ...
                    _errors.append(f"Unexpected error getting comments: {type(e_com_other).__name__}")

                return _details, _errors

            except HttpError as e_build: # Catch errors during build_google_api_client itself
                ...
                if status_code == 403:
                     if "quotaExceeded" in str(e_build):
                         ...
                         _errors.append("YouTube API access not configured or key invalid.")
                     else:
                ...
                logging.exception(f"Unexpected error initializing YouTube client for {video_id}")
                _errors.append(f"An unexpected error occurred during client init: {e_build_other}")
                return None, _errors
        ...
        logging.exception(f"Error running YouTube detail fetch in thread for {video_id}")
