...
def extract_urls_with_indices(text: str) -> List[Tuple[str, int]]:
    ...
    return [
        (match.group(0), match.start()) for match in GENERAL_URL_PATTERN.finditer(text)
    ]
...
def is_youtube_url(url: str) -> bool:
    ...
    return (
        domain in ("youtube.com", "www.youtube.com", "youtu.be")
        and YOUTUBE_URL_PATTERN.search(url) is not None
    )
...
def is_reddit_url(url: str) -> bool:
    ...
    domain = get_domain(url)
    return (
        domain in ("reddit.com", "www.reddit.com")
        and REDDIT_URL_PATTERN.search(url) is not None
    )
...
def add_field_safely(
    ...
    if (
        potential_size > MAX_EMBED_TOTAL_SIZE
        or len(current_embed.fields) >= MAX_EMBED_FIELDS
    ):
        # Current embed is full, finalize it and start a new one
        if (
            current_embed.fields
            or current_embed.description
            or (
                current_embed.title
                and current_embed.title != "Grounding Sources (cont.)"
            )
        ):  # Avoid adding empty embeds
...
def _truncate_base64_in_payload(
    payload: Any, max_len: int = 40, prefix_len: int = 10
) -> Any:
    """
        ...
        for key, value in payload.items():
            ...
            if key == "image_url" and isinstance(value, dict) and "url" in value:
                ...
                if (
                    ...
                    and url_value.startswith("data:image")
                    and ";base64," in url_value
                ):
                    try:
                        ...
                        if len(data) > max_len:
                            truncated_data = (
                                ...
                                + "..."
                                + data[-prefix_len:]
                                + f" (truncated {len(data)} chars)"
                            )
                            # Create a copy of the inner dict to modify
                            new_image_url_dict = value.copy()
                            new_image_url_dict["url"] = (
                                prefix + ";base64," + truncated_data
                            )
                        ...
                        new_dict[key] = _truncate_base64_in_payload(
                            value, max_len, prefix_len
                        )
                else:
            ...
            # Check for Gemini inline_data structure
                ...
                if (
                    isinstance(data_value, str) and len(data_value) > max_len
                ):  # Check if it's a string and long
                    # Create a copy of the inner dict to modify
                    new_inline_data_dict = value.copy()
                    # Truncate the data string itself
                    new_inline_data_dict["data"] = (
                        data_value[:prefix_len]
                        + "..."
                        + data_value[-prefix_len:]
                        + f" (truncated {len(data_value)} chars)"
                    )
