...
def is_message_allowed(message: discord.Message, config: Dict, is_dm: bool) -> bool:
    ...
    if isinstance(message.channel, discord.Thread) and message.channel.parent_id:
        ...
        if hasattr(message.channel.parent, 'category_id') and message.channel.parent.category_id:
            current_channel_ids.add(message.channel.parent.category_id)


    # User check
    allow_all_users = not allowed_user_ids and not allowed_role_ids # True if both allowed lists are empty
    
    user_is_explicitly_allowed = message.author.id in allowed_user_ids or any(id in allowed_role_ids for id in role_ids)
    ...
    if is_bad_user or not is_good_user: # If blocked OR (not allow_all_users AND not user_is_explicitly_allowed)
        return False

    # Channel check (only if not DM)
    if not is_dm:
        allow_all_channels = not allowed_channel_ids # True if allowed_channel_ids is empty
        
        channel_is_explicitly_allowed = any(id in allowed_channel_ids for id in current_channel_ids)
        ...
        if is_bad_channel or not is_good_channel: # If blocked OR (not allow_all_channels AND not channel_is_explicitly_allowed)
            return False
