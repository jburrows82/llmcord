...
from .utils import (
    extract_urls_with_indices, is_youtube_url, is_reddit_url, is_image_url,
...
async def fetch_external_content(
    ...
    for url, index in all_urls_with_indices:
        ...
        if is_youtube_url(url):
            fetch_tasks.append(fetch_youtube_data(url, index, config.get("youtube_api_key")))
        elif is_reddit_url(url):
            sub_id = extract_reddit_submission_id(url)
            if sub_id:
                fetch_tasks.append(fetch_reddit_data(url, sub_id, index, config.get("reddit_client_id"), config.get("reddit_client_secret"), config.get("reddit_user_agent")))
            else:
            ...
            async def fetch_image_url_content(img_url: str, img_idx: int) -> models.UrlFetchResult:
                try:
                    ...
                    async with httpx_client.stream("GET", img_url, timeout=15.0) as response:
            ...
            fetch_tasks.append(fetch_general_url_content(url, index)) # Assuming fetch_general_url_content uses its own client or a global one for now
    ...
    if use_google_lens:
        ...
        for i, attachment in enumerate(lens_images_to_process):
            ...
            try:
                ...
                lens_result = await process_google_lens_image(attachment.url, i, config) # Pass config here
