...
class MsgNode:
    ...
    lock: asyncio.Lock = field(default_factory=asyncio.Lock) # Lock for async access to this node
    full_response_text: Optional[str] = None # Stores the complete text from the LLM for this node (if it's an assistant response)
    # --- ADDED FIELD ---
    external_content: Optional[str] = None # Stores formatted fetched content (URLs, Lens) for this node
...
class UrlFetchResult:
    ...
    type: Literal["youtube", "reddit", "general", "google_lens_serpapi", "image_url_content"] = "general"
