...
import mimetypes # Added for image handling
...
async def generate_gemini_stream(
    ...
    for msg_data in history_for_api_call:
        ...
        try:
            ...
            yield None, None, None, f"Internal error creating Gemini content: {content_creation_error}", None, None
            return

    if not gemini_contents:
        logging.error("Gemini contents list is empty after processing history. Cannot make API call.")
        yield None, None, None, "Internal error: No valid content to send to Gemini.", None, None
        return
    ...
    try:
        ...
        async for chunk in stream_response:
            ...
            if hasattr(chunk, "prompt_feedback") and chunk.prompt_feedback and chunk.prompt_feedback.block_reason:
                ...
                yield text_chunk, finish_reason_str, grounding_meta, error_msg_chunk, None, None
                    ...
                    for rating in candidate.safety_ratings:
                        if rating.probability in (google_types.HarmProbability.MEDIUM, google_types.HarmProbability.HIGH):
                            if not text_chunk: # If blocked before any content
                                ...
                                yield None, finish_reason_str, grounding_meta, error_msg_chunk, None, None
                                return # Stop generation
...
async def generate_gemini_image_stream(
    ...
    for msg_data in history_for_api_call:
        ...
        parts_list = msg_data.get("parts", [])
        if not parts_list:
            logging.warning(f"Skipping message with no parts for Gemini image generation: Role {role}")
            continue
        try:
            # Ensure all parts are valid google_types.Part instances
            valid_parts = []
            for part_item in parts_list:
                ...
                # Add more conversions if other dict formats are expected for parts
                    ...
                    logging.warning(f"Skipping invalid part item for Gemini image generation: {type(part_item)}")
            
            if valid_parts:
                gemini_contents.append(google_types.Content(role=role, parts=valid_parts))
            else:
                logging.warning(f"Message with role '{role}' resulted in no valid parts for Gemini image generation.")
    ...
    try:
        ...
        if "config" in truncated_payload and isinstance(truncated_payload["config"], google_types.GenerateContentConfig):
            config_dict = {}
            # Iterate over known attributes of GenerateContentConfig
            for attr_name in ["candidate_count", "stop_sequences", "max_output_tokens", "temperature", "top_p", "top_k", "safety_settings", "response_modalities"]:
                if hasattr(truncated_payload["config"], attr_name):
                    attr_value = getattr(truncated_payload["config"], attr_name)
                    if attr_value is not None: # Only include if not None
                        config_dict[attr_name] = attr_value
        ...
        logging.error(f"Error during Gemini image generation payload logging: {e_log}", exc_info=True)
    # --- End Payload Logging ---
    ...
    try:
        ...
        async for chunk in stream_response:
            ...
            if hasattr(chunk, "prompt_feedback") and chunk.prompt_feedback and chunk.prompt_feedback.block_reason:
                error_msg_chunk = f"Prompt Blocked ({chunk.prompt_feedback.block_reason})"
                finish_reason_str = "safety"
                yield text_chunk, finish_reason_str, None, error_msg_chunk, image_data, image_mime_type
                return
                    ...
                    for rating in candidate.safety_ratings:
                        if rating.probability in (google_types.HarmProbability.MEDIUM, google_types.HarmProbability.HIGH):
                            if not text_chunk and not image_data:
                                error_msg_chunk = f"Response Blocked (Safety Rating: {rating.category}={rating.probability})"
                                finish_reason_str = "safety"
                                yield None, finish_reason_str, None, error_msg_chunk, None, None
                                return
