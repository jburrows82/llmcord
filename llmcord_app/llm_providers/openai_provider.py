...
async def generate_openai_stream(
    ...
    if current_provider_name not in PROVIDERS_SUPPORTING_USERNAMES:
        for msg_data in openai_messages:
            if msg_data.get("role") == "user" and "name" in msg_data:
                del msg_data["name"]
                logging.debug(f"Removed 'name' field for user message for provider '{current_provider_name}'")
    ...
    except APIError as e: # Catches various OpenAI API errors including 413, RateLimitError
