...
class MessageEditor:
    ...
    async def process_text_stream(
        self, stream_generator, new_msg, processing_msg, response_msgs,
        ...
        async for (text_chunk, finish_reason, chunk_grounding_metadata, 
            ...
            if error_message and error_message.startswith("COMPRESSION_INFO:"):
                try:
                    ...
                    compression_msg = error_message.replace("COMPRESSION_INFO:", "").strip()
                    ...
                    for field in base_embed.fields:
                        if not field.name.startswith("⚠️ The image is at") and not field.name.startswith("⚠️ the image is at"):
            ...
            if not use_plain_responses_config:
                await self._update_embed_messages(
                    new_msg, processing_msg, response_msgs, final_text,
            ...
            if finish_reason:
                ...
                break
        if should_retry:
            return {'success': False, 'text': '', 'response_msgs': response_msgs, 'should_retry': True}
    ...
    async def _update_embed_messages(
        self, new_msg, processing_msg, response_msgs, final_text,
        split_limit_config, base_embed, finish_reason, current_params,
        grounding_metadata, custom_search_queries_generated,
        successful_api_results_count, deep_search_used, attempt_num
    ):
        ...
        if start_next_msg or ready_to_edit or is_final_chunk:
            ...
            current_embed = await self._create_segment_embed(
                final_text, current_msg_idx, split_limit_config, base_embed,
            ...
            view_to_attach = None
            if is_final_chunk:
                is_successful = self._is_successful_finish(finish_reason, current_params['provider'])
                if is_successful:
    ...
    async def _create_segment_embed(
        ...
        current_display_text = final_text[
            current_msg_idx * split_limit_config:(current_msg_idx + 1) * split_limit_config
        ][:MAX_EMBED_DESCRIPTION_LENGTH]
        ...
        if is_final_chunk and is_blocked:
            ...
            current_embed.description = f"{stripped_text}\n\n{block_msg}" if stripped_text else block_msg
            current_embed.description = current_embed.description[:MAX_EMBED_DESCRIPTION_LENGTH]
    ...
    async def _send_or_edit_message(
        self, new_msg, processing_msg, response_msgs, current_embed,
        current_msg_idx, start_next_msg, is_final_chunk, final_text, view_to_attach
    ):
        ...
        if is_final_chunk and target_msg_for_node_update and target_msg_for_node_update.id in self.client.msg_nodes:
            self.client.msg_nodes[target_msg_for_node_update.id].full_response_text = final_text
    async def _send_plain_text_response(
        self, new_msg, processing_msg, response_msgs, final_text, split_limit_config,
        current_params, grounding_metadata, custom_search_queries_generated
    ):
    ...
    def _create_footer_text(
        self, current_params, attempt_num, grounding_metadata,
        custom_search_queries_generated, successful_api_results_count, deep_search_used
    ) -> str:
    ...
    def _create_response_action_view(
        self, new_msg, final_text, current_params, grounding_metadata,
        custom_search_queries_generated, successful_api_results_count
    ):
