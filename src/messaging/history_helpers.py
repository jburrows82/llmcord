...
async def process_message_attachments(
        ...
        if att.content_type:
            ...
            if att.content_type.startswith("text/"):
                ...
                if (is_target_provider_gemini and accept_files) or not is_target_provider_gemini:
                    is_relevant_for_download = True
            elif att.content_type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                is_relevant_for_download = True
            pass
    ...
    if current_role == "user":
        pass
        for att, resp in zip(attachments_to_fetch, attachment_responses):
            if att.content_type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                pass
                if isinstance(resp, httpx.Response) and resp.status_code == 200:
                    try:
                        from .utils import extract_text_from_docx_bytes
                        extracted_docx_text = await extract_text_from_docx_bytes(resp.content)
            ...
            try:
                if is_target_provider_gemini:
                    ...
                    base64_encoded_image = await asyncio.to_thread(
                        lambda b: base64.b64encode(b).decode("utf-8"), file_bytes_for_api
                    api_file_parts.append(
                        {
                            "type": "image_url",
                            "image_url": {
            ...
            for fetched_url_res in current_message_url_fetch_results:
                if (
                    ...
                    try:
                        ...
                        user_warnings.add(f"⚠️ Error processing image URL: {fetched_url_res.url[:50]}...")
