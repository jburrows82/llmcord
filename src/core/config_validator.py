...
class ConfigValidator:
    ...
    def validate_and_normalize_config(
        config_data: Dict[str, Any], filename: str
    ) -> Dict[str, Any]:
        """
    ...
    def _normalize_api_keys(
        provider_cfg: Dict[str, Any], provider_name: str, filename: str
    ):
    ...
    def _validate_int_config(
        config_data: Dict[str, Any], key: str, default: int, min_value: int = None
    ):
        ...
        if key not in config_data:
            ...
            try:
                ...
                if min_value is not None and val < min_value:
                    logger.warning(
                        f"'{key}' ({val}) must be >= {min_value}. Using default: {default}"
                    )
                    config_data[key] = default
                else:
                    config_data[key] = val
                ...
                logger.warning(
                    f"'{key}' is not a valid integer. Using default: {default}"
                )
                config_data[key] = default

    @staticmethod
    def _validate_float_config(
        config_data: Dict[str, Any], key: str, default: float, min_value: float = None
    ):
        ...
        if key not in config_data:
            ...
            try:
                ...
                logger.warning(
                    f"'{key}' is not a valid float. Using default: {default}"
                )
                config_data[key] = default
    ...
    def _validate_searxng_config(config_data: Dict[str, Any]):
        ...
        if GROUNDING_SYSTEM_PROMPT_CONFIG_KEY not in config_data or not config_data.get(
            GROUNDING_SYSTEM_PROMPT_CONFIG_KEY
        ):
            logger.info(
                f"'{GROUNDING_SYSTEM_PROMPT_CONFIG_KEY}' not found or empty. Using default."
            )
    ...
    def _validate_model_selections(config_data: Dict[str, Any]):
        ...
        if not config_data.get(FALLBACK_MODEL_SYSTEM_PROMPT_CONFIG_KEY):
            logger.info(
                f"'{FALLBACK_MODEL_SYSTEM_PROMPT_CONFIG_KEY}' not found or empty. Using default."
            )
        ...
        if GROUNDING_MODEL_THINKING_BUDGET_VALUE_CONFIG_KEY in config_data:
            try:
                ...
                if not (
                    ...
                    logger.warning(
                        f"'{GROUNDING_MODEL_THINKING_BUDGET_VALUE_CONFIG_KEY}' ({val}) is outside valid range. Using default: {GROUNDING_MODEL_DEFAULT_THINKING_BUDGET_VALUE}"
                    )
                ...
                logger.warning(
                    f"'{GROUNDING_MODEL_THINKING_BUDGET_VALUE_CONFIG_KEY}' is not a valid integer. Using default: {GROUNDING_MODEL_DEFAULT_THINKING_BUDGET_VALUE}"
                )
                config_data[GROUNDING_MODEL_THINKING_BUDGET_VALUE_CONFIG_KEY] = (
                    GROUNDING_MODEL_DEFAULT_THINKING_BUDGET_VALUE
                )
    ...
    @staticmethod
    def _validate_float_range_config(
        ...
        if key not in config_data:
            ...
            try:
                ...
                if not (min_val <= val <= max_val):
                    logger.warning(
                        f"'{key}' ({val}) must be between {min_val} and {max_val}. Using default: {default}"
                    )
                    config_data[key] = default
                else:
                    config_data[key] = val
    ...
    def _validate_output_sharing_config(config_data: Dict[str, Any]):
        ...
        if OUTPUT_SHARING_CONFIG_KEY not in config_data:
            ...
            logger.info(
                f"'{OUTPUT_SHARING_CONFIG_KEY}' section not found. Using defaults."
            )

        output_sharing_cfg = config_data[OUTPUT_SHARING_CONFIG_KEY]
        if not isinstance(output_sharing_cfg, dict):
            logger.warning(
                f"'{OUTPUT_SHARING_CONFIG_KEY}' section is not a dictionary. Using defaults."
            )
            output_sharing_cfg = {}
        ...
        if TEXTIS_ENABLED_CONFIG_KEY not in output_sharing_cfg:
            ...
            logger.info(
                f"'{TEXTIS_ENABLED_CONFIG_KEY}' not found. Defaulting to False."
            )
        ...
        if URL_SHORTENER_ENABLED_CONFIG_KEY not in output_sharing_cfg:
            ...
            logger.info(
                f"'{URL_SHORTENER_ENABLED_CONFIG_KEY}' not found. Defaulting to False."
            )
        ...
        if URL_SHORTENER_SERVICE_CONFIG_KEY not in output_sharing_cfg:
            ...
            logger.info(
                f"'{URL_SHORTENER_SERVICE_CONFIG_KEY}' not found. Defaulting to 'tinyurl'."
            )
        elif (
            not isinstance(output_sharing_cfg[URL_SHORTENER_SERVICE_CONFIG_KEY], str)
    ...
    def _validate_url_extractor_config(config_data: Dict[str, Any]):
        ...
        # Main extractor
        if (
            config_data.get(MAIN_GENERAL_URL_CONTENT_EXTRACTOR_CONFIG_KEY)
            not in VALID_URL_EXTRACTORS
        ):
            logger.warning(
                f"Invalid value for '{MAIN_GENERAL_URL_CONTENT_EXTRACTOR_CONFIG_KEY}'. Using default: {DEFAULT_MAIN_GENERAL_URL_CONTENT_EXTRACTOR}"
            )
        ...
        # Fallback extractor
        if (
            config_data.get(FALLBACK_GENERAL_URL_CONTENT_EXTRACTOR_CONFIG_KEY)
            not in VALID_URL_EXTRACTORS
        ):
            logger.warning(
                f"Invalid value for '{FALLBACK_GENERAL_URL_CONTENT_EXTRACTOR_CONFIG_KEY}'. Using default: {DEFAULT_FALLBACK_GENERAL_URL_CONTENT_EXTRACTOR}"
            )
        ...
        if config_data.get(JINA_ENGINE_MODE_CONFIG_KEY) not in VALID_JINA_ENGINE_MODES:
            logger.warning(
                f"Invalid value for '{JINA_ENGINE_MODE_CONFIG_KEY}'. Using default: {DEFAULT_JINA_ENGINE_MODE}"
            )
        ...
        # Crawl4AI cache mode
        if (
            config_data.get(CRAWL4AI_CACHE_MODE_CONFIG_KEY)
            not in VALID_CRAWL4AI_CACHE_MODES
        ):
            logger.warning(
                f"Invalid value for '{CRAWL4AI_CACHE_MODE_CONFIG_KEY}'. Using default: {DEFAULT_CRAWL4AI_CACHE_MODE}"
            )
    ...
    def _validate_web_content_api_config(config_data: Dict[str, Any]):
        ...
        if (
            not isinstance(
                config_data.get(WEB_CONTENT_EXTRACTION_API_URL_CONFIG_KEY), str
            )
            or not config_data.get(
                WEB_CONTENT_EXTRACTION_API_URL_CONFIG_KEY, ""
            ).strip()
        ):
            logger.warning(
                f"'{WEB_CONTENT_EXTRACTION_API_URL_CONFIG_KEY}' is not a non-empty string. Using default: {DEFAULT_WEB_CONTENT_EXTRACTION_API_URL}"
            )
    ...
    def _validate_text_limits_config(config_data: Dict[str, Any], filename: str):
        ...
        if MAX_TEXT_LIMITS_CONFIG_KEY not in config_data:
            logger.warning(
                f"'{MAX_TEXT_LIMITS_CONFIG_KEY}' section not found in {filename}. Using hardcoded default of 128000 for all models."
            )
        ...
        if not isinstance(limits_config, dict):
            logger.warning(
                f"'{MAX_TEXT_LIMITS_CONFIG_KEY}' is not a dictionary. Using hardcoded default of 128000."
            )
            config_data[MAX_TEXT_LIMITS_CONFIG_KEY] = {
                DEFAULT_MAX_TEXT_KEY: 128000,
                MODEL_SPECIFIC_MAX_TEXT_KEY: {},
        ...
        if MODEL_SPECIFIC_MAX_TEXT_KEY not in limits_config:
            ...
            logger.warning(
                f"'{MODEL_SPECIFIC_MAX_TEXT_KEY}' is not a dictionary. No model-specific limits will be applied."
            )
            limits_config[MODEL_SPECIFIC_MAX_TEXT_KEY] = {}
        else:
        ...
        if "max_text" in config_data:
            logger.warning(
                f"Old 'max_text' setting found in {filename}. It is now ignored. Please use '{MAX_TEXT_LIMITS_CONFIG_KEY}' instead."
            )
    ...
    def _validate_misc_config(config_data: Dict[str, Any]):
        ...
        if not config_data.get(
            PROMPT_ENHANCER_SYSTEM_PROMPT_CONFIG_KEY
        ) or not isinstance(config_data[PROMPT_ENHANCER_SYSTEM_PROMPT_CONFIG_KEY], str):
            logger.info(
                f"'{PROMPT_ENHANCER_SYSTEM_PROMPT_CONFIG_KEY}' not found or invalid. Using default."
            )
