...
class ConfigLoader:
    ...
    @staticmethod
    async def load_config(filename: str = "config/config.yaml") -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            config_data = ConfigValidator.validate_and_normalize_config(config_data, filename)
    ...
    def _load_alternative_search_config(config_data: Dict[str, Any]):
        ...
        if ALT_SEARCH_SECTION_KEY not in config_data:
            ...
            logger.info(f"'{ALT_SEARCH_SECTION_KEY}' section not found. Creating with defaults.")

        alt_search_config = config_data[ALT_SEARCH_SECTION_KEY]
        if not isinstance(alt_search_config, dict):
            logger.warning(f"'{ALT_SEARCH_SECTION_KEY}' section is not a dictionary. Using defaults.")
            alt_search_config = {}
        ...
        if ALT_SEARCH_ENABLED_KEY not in alt_search_config:
            ...
            logger.info(f"'{ALT_SEARCH_ENABLED_KEY}' not found in '{ALT_SEARCH_SECTION_KEY}'. Defaulting to False.")
        elif not isinstance(alt_search_config[ALT_SEARCH_ENABLED_KEY], bool):
            logger.warning(f"'{ALT_SEARCH_ENABLED_KEY}' in '{ALT_SEARCH_SECTION_KEY}' is not a boolean. Defaulting to False.")
            alt_search_config[ALT_SEARCH_ENABLED_KEY] = False

        # Validate prompt template
        if (ALT_SEARCH_PROMPT_KEY not in alt_search_config or 
            not alt_search_config.get(ALT_SEARCH_PROMPT_KEY) or
            not isinstance(alt_search_config[ALT_SEARCH_PROMPT_KEY], str)):
            alt_search_config[ALT_SEARCH_PROMPT_KEY] = DEFAULT_ALT_SEARCH_PROMPT_TEMPLATE
            logger.info(f"'{ALT_SEARCH_PROMPT_KEY}' not found, empty, or invalid in '{ALT_SEARCH_SECTION_KEY}'. Using default template.")
...
def get_max_text_for_model(config: Dict[str, Any], model_identifier: str) -> int:
    ...
    safety_margin = config.get(MAX_TEXT_SAFETY_MARGIN_CONFIG_KEY, DEFAULT_MAX_TEXT_SAFETY_MARGIN)
