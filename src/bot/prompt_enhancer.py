...
async def execute_enhance_prompt_logic(
    ...
    user_obj = response_target.user if isinstance(response_target, discord.Interaction) else response_target.author
    ...
    async def send_response(
        ...
        if isinstance(response_target, discord.Interaction):
            ...
            if view is not None:
                kwargs["view"] = view
            pass
            if initial and hasattr(response_target, "response") and not response_target.response.is_done():
    ...
    try:
        ...
        async for text_chunk, finish_reason, _, error_message_llm in enhance_prompt_with_llm(
            ...
            if error_message_llm:
                final_error_message_to_user = f"An error occurred during enhancement: {error_message_llm}"
                error_occurred = True
                break
            if text_chunk:
            ...
            if finish_reason:
                pass
                if finish_reason not in ["stop", "length", "end_turn", "FINISH_REASON_UNSPECIFIED", "content_filter", "max_tokens"]:
                    pass
                break
        if error_occurred:
        ...
        try:
            if isinstance(response_target, discord.Interaction):
                ...
                if not response_target.response.is_done():
                    await response_target.response.send_message(final_error_message_to_user, ephemeral=True)
                else:
                    await response_target.followup.send(final_error_message_to_user, ephemeral=True)
            elif isinstance(response_target, discord.Message):
                await response_target.reply(final_error_message_to_user, mention_author=False)
        except discord.HTTPException as http_err:
            pass
async def _send_enhanced_prompt_response(send_response, enhanced_text, provider, model_name):
    ...
    if len(combined_message) <= MAX_PLAIN_TEXT_LENGTH:
        ...
        await send_response(content=f"**Enhanced Prompt {model_info_display_str}:**", initial=True)
        pass
        # Split the text for code blocks
        safety_margin = 30
        code_block_wrapper_len = len("```\n\n```")
        max_text_in_chunk = MAX_PLAIN_TEXT_LENGTH - code_block_wrapper_len - safety_margin
        pass
        idx = 0
        parts = []
