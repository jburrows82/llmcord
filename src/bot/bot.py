...
class LLMCordClient(discord.Client):
    ...
    async def setup_hook(self):
        ...
        commands_to_register = [
            ...
            ("systemprompt", "Set your custom system prompt for the bot.", set_system_prompt_command),
            ("setgeminithinking", "Toggle usage of the 'thinkingBudget' parameter for Gemini models.", setgeminithinking),
            ("help", "Displays all available commands and how to use them.", help_command),
            ("enhanceprompt", "Enhances a given prompt using an LLM.", enhance_prompt_command),
        ]
    ...
    async def on_message(self, new_msg: discord.Message):
        ...
        if not should_handle:
            ...
            if MessageRouter.is_prefix_command(new_msg, PrefixCommandHandler.ENHANCE_CMD_PREFIX):
                if MessageRouter.check_permissions(new_msg, self.config, is_dm):
                    await PrefixCommandHandler.handle_enhance_prompt_command(new_msg, self)
            return
        ...
        try:
            if await self._handle_special_messages(new_msg, original_content, processing_msg, start_time):
                return
            ...
            try:
                await new_msg.channel.send(f"❌ An error occurred while processing your message: {str(e)}")
            except Exception:
    ...
    async def _handle_deep_search(
        ...
        modified_content, topic = await DeepSearchHandler.process_deep_search_query(
            new_msg, cleaned_content, processing_msg, use_plain_for_initial_status, self.config
        )
        ...
        await StandardMessageHandler.process_standard_message(
            modified_msg, modified_content, processing_msg, self, start_time,
    ...
    def _create_modified_message(self, original_msg: discord.Message, new_content: str):
        ...
        class ModifiedMessage:
            def __init__(self, original, content):
                ...
                for attr in ['author', 'channel', 'guild', 'id', 'attachments', 'reference',
                           'created_at', 'edited_at', 'mention_everyone', 'mentions',
                           'channel_mentions', 'role_mentions', 'pinned', 'flags', 'type']:
                ...
                optional_attrs = ['embeds', 'reactions', 'webhook_id', 'application_id',
                                'activity', 'application', 'stickers', 'components', 'thread',
                                'interaction', 'role_subscription', 'resolved', 'position',
                                'poll', 'call', 'system_content']
                for attr in optional_attrs:
                    setattr(self, attr, getattr(original, attr, [] if attr.endswith('s') else None))
    ...
    async def retry_with_modified_content(
        ...
        try:
            ...
            retry_message = self._create_modified_message(original_message, modified_content)
            await self.on_message(retry_message)
        except Exception as e:
            logging.error(f"Error in retry_with_modified_content: {e}", exc_info=True)
            try:
                await original_message.channel.send(f"❌ Failed to retry the request: {str(e)}")
            except Exception:
                pass
