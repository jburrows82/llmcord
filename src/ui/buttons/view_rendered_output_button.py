    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not view.full_response_text:
            await self.handle_interaction_error(
                interaction, "No response text available to render."
            )
            return
        if not view.app_config:
            await self.handle_interaction_error(
                interaction, "Application configuration is not available for rendering."
            )
            return
        ...
        if not textis_is_enabled:
            await self.handle_interaction_error(
                interaction,
                "Output sharing (text.is) is not enabled in the configuration.",
            )
            return
        if not success:
        ...
        try:
            ...
            msg = (
                f"🔗 View output on text.is: {public_url}"
                if public_url
                else "Could not generate a public link via text.is for the output."
            )
            if success:
