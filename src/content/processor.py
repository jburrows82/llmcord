    ...
    try:
        ...
        if referenced_msg and referenced_msg.content:
            ...
            if urls_with_indices:
                ...
                logging.info(
                    f"Found {len(urls)} URL(s) in replied-to message {referenced_msg.id}: {urls}"
                )
                return urls_content

    except (discord.NotFound, discord.HTTPException) as e:
        logging.warning(
            f"Could not fetch referenced message {new_msg.reference.message_id}: {e}"
        )
    except Exception as e:
        logging.error(
            f"Unexpected error extracting URLs from referenced message: {e}",
            exc_info=True,
        )
...
async def process_content_and_grounding(
    ...
    if replied_urls_content:
        combined_content_for_url_extraction = (
            f"{cleaned_content} {replied_urls_content}".strip()
        )
        logging.info(
            f"Added URLs from replied-to message to content processing. Combined content: {len(combined_content_for_url_extraction)} chars"
        )

    # Re-extract URLs from combined content
    all_urls_in_combined_content = extract_urls_with_indices(
        combined_content_for_url_extraction
    )
    ...
    if user_has_provided_urls:
        ...
        for url, index_pos in all_urls_in_combined_content:
            char_before_is_backtick = (
                index_pos > 0
                and combined_content_for_url_extraction[index_pos - 1] == "`"
            )
    ...
    if use_google_lens:
        ...
        if is_alt_search_enabled:
            ...
            elif model_name == "gemini-2.0-flash-preview-image-generation" or model_name.startswith("imagen-"):
