...
def _process_history_to_contents(history_for_api_call: List[Dict[str, Any]]) -> List[google_types.Content]:
    ...
    for msg_data in history_for_api_call:
        ...
        try:
            ...
            logging.error(f"Failed to create google_types.Content for Gemini. Role: {role}", exc_info=True)
...
def _create_safety_settings(app_config: Dict[str, Any]) -> List[google_types.SafetySetting]:
...
def _create_thinking_config(thinking_budget_val: Any) -> Optional[google_types.ThinkingConfig]:
...
def _log_payload(payload_to_log: Dict[str, Any], is_image_generation: bool = False) -> None:
    ...
    try:
        ...
        if "config" in truncated_payload and isinstance(
            ...
            config_dict = {}
            attrs = ["candidate_count", "stop_sequences", "max_output_tokens", "temperature", 
                    "top_p", "top_k", "safety_settings", "tools", "tool_config", 
                    "system_instruction", "thinking_config", "response_modalities"]
        ...
        if hasattr(candidate.content, "parts") and candidate.content.parts:
            for part in candidate.content.parts:
                if hasattr(part, "text") and part.text:
                    ...
                    logging.info(f"Received image data: {len(image_data)} bytes, MIME type: {image_mime_type}")
...
async def _make_gemini_request(
    llm_client, model_name: str, contents: List[google_types.Content], 
...
async def generate_gemini_stream(
    ...
    try:
        ...
        thinking_config = _create_thinking_config(gemini_extra_params.pop("thinking_budget", None))
        ...
        response = await _make_gemini_request(llm_client, model_name, gemini_contents, api_generation_config)

        # Check for prompt feedback blocking
        if (hasattr(response, "prompt_feedback") and response.prompt_feedback and 
            response.prompt_feedback.block_reason):
            yield None, "safety", None, f"Prompt Blocked ({response.prompt_feedback.block_reason})", None, None
            return
        ...
        if hasattr(response, "candidates") and response.candidates:
            ...
            grounding_meta = getattr(candidate, "grounding_metadata", None) if hasattr(candidate, "grounding_metadata") else None
            ...
            if full_text:
                ...
                yield None, finish_reason_str or "stop", grounding_meta, None, None, None
...
async def generate_gemini_image_stream(
    ...
    try:
        ...
        if hasattr(response, "candidates") and response.candidates:
            candidate = response.candidates[0]
            full_text, image_data, image_mime_type = _process_response_content(candidate)
            finish_reason_str = _get_finish_reason(candidate)
            
            safety_error = _check_safety_ratings(candidate, bool(full_text or image_data))
            if safety_error:
        ...
        yield None, None, None, f"Gemini Image Generation API Error: {type(e).__name__}", None, None
    except Exception as e:
